...
> [!WARNING]
> Resample **only the training set**, and only after splitting. Applying SMOTE or oversampling before the train/test split leaks synthetic copies of minority samples into the test set, producing evaluation scores that look excellent and collapse in production.
...
> [!WARNING]
> Scalers and imputers must be **fitted on the training set only**, then applied to validation and test data. Fitting a StandardScaler on the full dataset lets test-set statistics bleed into training — the same leakage problem as resampling before the split.
...
> [!WARNING]
> DataBrew and SageMaker Data Wrangler are the classic distractor pair. DataBrew is a **standalone Glue tool for general data cleaning**, aimed at analysts and outputting to S3. Data Wrangler lives **inside SageMaker Studio** and exports straight into an ML workflow (Processing, Pipelines, Feature Store). If the scenario ends in a training job, it is Data Wrangler.
